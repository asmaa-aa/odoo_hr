...
class EloHrModePay(models.Model):
    _name = 'elo.hr.payement.mode'
...
class HrContract(models.Model):
    ...
    def action_view_contract_wizard(self):
        ...
        context = {
            ...
            'default_contract_ids': [(6, 0, contracts.ids)] if contracts else False,
        }

        return {
            'name': 'Contract Report Wizard',
    ...
    dispositif = fields.Many2one('elo.hr.dispositif',string='Dispositif',)
    contrat = fields.Many2one('elo.hr.contract.anem',string='Contrat',)
    type_contrat = fields.Many2one('elo.hr.contract.type.anem',string='Type de Contrat',)
    ...
    payement_mode = fields.Many2one(
        'elo.hr.payement.mode', 
        string='Mode de paiement',
        default=lambda self: self.env['elo.hr.payement.mode'].search([], limit=1))

    motif1 = fields.Many2one('elo.hr.reason.end.contract', string='Motif de fin de contrat',)
    ...
    iep = fields.Many2one('elo.pay.iep', string="IEP")
        ...
        if res.structure_type_id.name not in ['Consultant','Stagiaire']:
            ...
            if not allocation:
                ...
                allocation_conge = self.env['hr.leave.allocation'].sudo().create({
                        ...
                        'accrual_plan_id':rule.id,
                        'employee_id': res.employee_id.id,
                        'number_of_days_display': 0,
                        'number_of_hours_display': 0,
                        'number_of_days': 0,})

                #allocation_conge.employee_ids= [(5, 0, 0)]
                #allocation_conge.employee_ids = [(4, res.employee_id.id)]
                allocation_conge.employee_id = res.employee_id.id
        return res
    ...
    @api.model
    def remove_worker_type(self):
        res = worker_structure_type = self.env['hr.payroll.structure.type'].search([('name', '=', 'Worker')]).unlink() 
        
 

# Motif de fin de contrat
class EloHrReasonEndContract(models.Model): 
    _name = 'elo.hr.reason.end.contract'
    _description = 'Motif de fin de contrat'
    name = fields.Char(string='Name')

# Dispositif ANEM
class EloHrDispositif(models.Model):
    _name = 'elo.hr.dispositif'
    ...
    def elo_name_get(self):
        res = super(HrDispositif, self).elo_name_get()
        data1 = []
        for record in self:
...
class EloHrContractAnem(models.Model):
    _name = 'elo.hr.contract.anem'
    ...
    @api.depends('name','code')
    def elo_name_get(self):
        res = super(EloHrContractAnem, self).elo_name_get()
        data2 = []
        for record in self:
            if record.name and record.code:
                display_value = record.code +":" +record.name 
...
class EloHContratrTypeAnem(models.Model):
    _name = 'elo.hr.contract.type.anem'
    ...
    @api.depends('name','code')
    def elo_name_get(self):
        res = super(EloHContratrTypeAnem, self).elo_name_get()
        data3 = []
        for record in self:
            if record.name and record.code:
                display_value = record.code +":" +record.name 
